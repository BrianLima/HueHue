//Based on Adrilight's class for SerialStream
//https://github.com/fabsenet/adrilight/blob/master/adrilight/SerialStream.cs
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    while (!cancellationToken.IsCancellationRequested)
                    {
                        ...
                        int delayInMs = 1;//Math.Max(minTimespan, Settings.MinimumRefreshRateMs - (int)_stopwatch.ElapsedMilliseconds);
                        if (delayInMs > 0)
                        {
